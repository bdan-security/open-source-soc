...
> **Figure A.1:** Kali Linux executing a SYN scan against the SME asset to identify exposed network services.
...
> **Figure A.2:** The Wazuh dashboard successfully detecting and classifying the Nmap port scan as a network security alert.
...
* Threat detection performance measurement

# 🔍 Key Detection Logic

The detection process can be summarised as:

```text id="0y5w7r"
Attacker performs reconnaissance
            │
            ▼
Nmap generates multiple TCP SYN packets
            │
            ▼
Suricata monitors network traffic
            │
            ▼
Custom rule identifies repeated SYN activity
            │
            ▼
...
Wazuh Agent collects the event
            │
            ▼
Wazuh Manager correlates the telemetry
            │
            ▼
Security alert displayed in the SIEM
```
...
```text id="r9p70e"
../Images/Scenario_A_Nmap_Scan.png
../Images/Scenario_A_Wazuh_Detection.png
```
...
│
├── Images/
│   ├── Scenario_A_Nmap_Scan.png
│   └── Scenario_A_Wazuh_Detection.png
│
├── 01_Lab_Setup/
│   └── README.md
│
